  ...
  override fun start() {
    ...
    mqttClient?.connect(MQTT_SERVER_PORT, MQTT_SERVER_HOST) { res: AsyncResult<MqttConnAckMessage?> ->
      if (res.succeeded()) {
        ...
        }.subscribe("/cc3200/TMP006", 0)
          .pingResponseHandler {
            log4vertx.info(eb, "Rec ping from mqtt!")
            eb.publish(CONFIG_MICRO_IOT_MQTT_QUEUE,
              JsonObject()
                .put("topic", "/cc3200/ToggleLEDCmdL2")
                .put("message", entries.toString()))
          }
